...
#------------------------------------------------------------------------------
# Functions for Generating Meshes from Geometry
#------------------------------------------------------------------------------
...
def extrude_shape(X, A, B, outline=10, outline_normals=None, scale=1, closed=True,
    ...
    T = norm(np.roll(X, -1, axis=0) - np.roll(X, +1, axis=0))
    ds = mag1(np.roll(X, -1, axis=0) - X)
    ...
    handedness = np.sign(dot(X[1] - X[0], cross(A[0], B[0])))
    ...
    if enforce_direction and outline_closed:
        ...
        #print dot(norm(X[1] - X[0]), cross(A[0], B[0]))
        #print handedness
    ...
    if outline_normals is None:
        if outline_closed:
            t = norm(np.roll(outline, -1, axis=1) - outline)
            seg_n = t[..., ::-1] * (-1, 1) * -handedness
            outline_normals = norm(seg_n + np.roll(seg_n, +1, axis=1))
        else:
            t = norm(outline[:, 1:] - outline[:, :-1])
            seg_n = t[..., ::-1] * (-1, 1) * -handedness
            outline_normals = np.zeros_like(outline)
            outline_normals[:, :-1] += seg_n
            outline_normals[:, 1:] += seg_n
            outline_normals = norm(outline_normals)
    ...
    R = offset - T*dot1(T, offset)
    r = mag1(R) #Radius of point
    ...
    normals = norm(normals - S*dot1(S, normals))
